...
type side = { man : bool; goat : bool; cabbage : bool; wolf : bool }
type state = side * side

let safe (left, right) =
  let safe_side side =
    side.man || (not side.goat)
    || (side.goat && ((not side.cabbage) || (side.cabbage && side.wolf)))
  in
  left.man <> right.man && left.goat <> right.goat
  && left.cabbage <> right.cabbage
  && left.wolf <> right.wolf && safe_side left && safe_side right
...
let step mov (left, right) =
  ...
  let move dep arr =
    ...
    | Goat ->
        if dep.goat then
          Just
            ( { dep with man = false; goat = false },
              { arr with man = true; goat = true } )
        else Nothing
    | Wolf ->
        if dep.wolf then
          Just
            ( { dep with man = false; wolf = false },
              { arr with man = true; wolf = true } )
        else Nothing
    | Cabbage ->
        if dep.cabbage then
          Just
